 ...
 */
PIDController2481::PIDController2481(float Kp, float Ki, float Kd,
 ...
 * integral and differental terms. The default is 50ms.
 */
PIDController2481::PIDController2481(float Kp, float Ki, float Kd, float Kf,
								PIDSource *source, PIDOutput *output,
								float period) :
	m_semaphore (0)
{
...
void PIDController2481::Initialize(float Kp, float Ki, float Kd, float Kf,
								PIDSource *source, PIDOutput *output,
								float period)
{
	m_table = NULL;
	...
	m_controlLoop = new Notifier(PIDController2481::CallCalculate, this);
 ...
 */
void PIDController2481::CallCalculate(void *controller)
{
	PIDController2481 *control = (PIDController2481*) controller;
	control->Calculate();
}

void PIDController2481::ResetError() {
			...
			if(m_I != 0)
			{
				...
				if (potentialIGain < m_maximumOutput)
				{
					if (potentialIGain > m_minimumOutput)
						...
						m_totalError = m_minimumOutput;
				}
				else
				{
					m_totalError = m_maximumOutput;
				}
			}
 ...
 */
void PIDController2481::SetPID(float p, float i, float d)
{
	CRITICAL_REGION(m_semaphore)
	{
		m_P = p;
 ...
 */
void PIDController2481::SetPID(float p, float i, float d, float f)
{
	CRITICAL_REGION(m_semaphore)
	{
		m_P = p;
 ...
 */
void PIDController2481::SetInputRange(float minimumInput, float maximumInput)
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
void PIDController2481::SetOutputRange(float minimumOutput, float maximumOutput)
{
	CRITICAL_REGION(m_semaphore)
	{
...
/**
 * Set the setpoint for the PIDController2481
 * @param setpoint the desired setpoint
 */
void PIDController2481::SetSetpoint(float setpoint)
{
	CRITICAL_REGION(m_semaphore)
	{
...
/**
 * Returns the current setpoint of the PIDController2481
 * @return the current setpoint
 */
float PIDController2481::GetSetpoint()
{
	float setpoint;
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
float PIDController2481::GetError()
{
	float error;
  double pidInput;
	CRITICAL_REGION(m_semaphore)
 ...
 */
void PIDController2481::SetTolerance(float percent)
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 * @param percentage error which is tolerable
 */
void PIDController2481::SetPercentTolerance(float percent)
{
	CRITICAL_REGION(m_semaphore)
	{
		m_toleranceType = kPercentTolerance;
 ...
 * @param percentage error which is tolerable
 */
void PIDController2481::SetAbsoluteTolerance(float absTolerance)
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
bool PIDController2481::OnTarget()
{
	bool temp;
...
/**
 * Begin running the PIDController2481
 */
...
/**
 * Stop running the PIDController2481, this sets the output to zero before stopping.
 */
...
/**
 * Return true if PIDController2481 is enabled.
 */
bool PIDController2481::IsEnabled()
{
	bool enabled;
	CRITICAL_REGION(m_semaphore)
	{
...
std::string PIDController2481::GetSmartDashboardType(){
	return "PIDController";
}

void PIDController2481::InitTable(ITable* table){
	if(m_table!=NULL)
...
ITable* PIDController2481::GetTable(){
	return m_table;
}

void PIDController2481::ValueChanged(ITable* source, const std::string& key, EntryValue value, bool isNew){
...
void PIDController2481::UpdateTable() {

}

void PIDController2481::StartLiveWindowMode() {
	Disable();
}
