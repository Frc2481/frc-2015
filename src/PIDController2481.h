 ...
 * PIDOutput
 */
class PIDController2481 : public LiveWindowSendable, public Controller, public ITableListener
{
public:
	PIDController2481(float p, float i, float d, PIDSource *source, PIDOutput *output, float period =
			0.05);
	PIDController2481(float p, float i, float d, float f, PIDSource *source, PIDOutput *output,
			float period = 0.05);
	virtual ~PIDController2481();
	virtual float Get();
...
protected:
	...
	DISALLOW_COPY_AND_ASSIGN(PIDController2481);
};
